...
public class HomeController {
	 ...
	 * 
	 * 만약 VO에 설정된 유효성 검사가 한개라도 통과하지 못하면 
	 * BindingResult 객체에 오류메시지가 포함된다.
	 * BindingResult 객체의 hasErrors() method를 사용하여
	 * 오류가 있는지 확인하고
	 * 다시 원래 form(*.jsp)로 되돌려 보낸다.
	 * 
	 * @Valid 의 BindingResult 는 순서를 바꾸면 안된다.
	 * 
	 * 보통 파일 업로드를 받을때는 파일 정보를 수신할 매개변수에
	 * @RequestParam 을 사용한다.
	 * 
	 * 하지만 여러파일을 수신하는 MultipartHttpServletRequest
	 * 
	 * 
	 */
	 ...
	 *    내부 오류가 발생한다. 
	 *    		이때, required 를 false로 선언하고
	 *    		defalutValue 를 설정하면 오류를 일부 방지할 수 있다.
	 */
	@ResponseBody
	@RequestMapping(value = "/num", method = RequestMethod.GET)
	public String nums(@Valid @RequestParam(name = "num", 
								     		required = false, 
