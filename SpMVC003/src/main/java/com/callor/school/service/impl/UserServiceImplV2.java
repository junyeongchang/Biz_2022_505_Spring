...
public class UserServiceImplV2 extends UserServiceImplV1 {
	...
	public UserVO login(UserVO userVO) {
		...
		// Select 한 데이터가 null 이면 username이 없다
		// 잘못입력했거나 아직 회원가입하지 않은 상태
		// Select 한 데이터가 null 아니면 username 있다.
		if (joinUserVO != null) {
			log.debug("JOIN : " + joinUserVO.toString());
			// username 으로 조회한 데이터가 있으면
			// DB에 저장된 password 와 입력한 password 비교
			boolean bYes = pass.matches(password, joinUserVO.getPassword());
			// 비밀번호가 일치하지 않으면
			// null을 return 하고 종료하기
			if (bYes == false) {
				return null;
			}
		}
		// 정상 로그인 되면 joinUserVO 에는 사용자 정보데이터가 담기고
