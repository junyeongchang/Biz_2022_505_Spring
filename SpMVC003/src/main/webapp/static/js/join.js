document.addEventListener("DOMContentLoaded", () => {
  ...
  const username_isBlank = () => {
    ...
    // div.username 항목에 warn 이라는 class를 덫붙여라
    /*
        JS 코드르 사용하여 여러가지 style을 변경하고자 할때
        일일이 style 항목을 변경하는 방법이 있지만
        미리 본문의 style 항목에 사용하지 않는 class 하나 만들어 두고 속성을 미리 작성해 놓는다
  ...
  if (id_check) {
    id_check.addEventListener("click", () => {
      ...
      // queryString 방식으로 요청
      // localhost:8080/school/user/idcheck?user=callor
      // URL(주소창)에 서버보내는 변수이름(username)이 노출되는 문제가 있다
      // 보내는 데이터의 길이 제한이 있다.

      // URL, PathWarriable 방식으로 데이터(검사하고자 하는 username)를 전송하기
      // REST Ful 방식이라고 한다.
