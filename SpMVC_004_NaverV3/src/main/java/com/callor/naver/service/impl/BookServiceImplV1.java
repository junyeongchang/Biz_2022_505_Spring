...
public class BookServiceImplV1 implements BookService{
	...
	/*
	 * GenericDao 를 상속받아 BookDao를 만들고,
	 * BookDao를 상속받아 BookService를 만들었다.
	 * 이러한 다중상속관계에서
	 * BookDao를 사용하여 서비스를 구현하려고 한다.
	 * 보통 Dao를 설정할때 private final로 선언하고
	 * 생성자에서 주입받아 사용했다.
	 * 
	 * 그런데 다중상속 관계에서
	 ...
	 * method르 만들고 method에 @Autowired를 설정해 두면
	 * 서비스가 처음 실행될때 method를 자동으로 실행해 준다.
	 */
	@Autowired
	@Override
	public void create_book_table() {
		...
		/*
		 * BookDao의 create_book_table() method를 실행하라
		 * 만약 exception 이 발생하면 무시하라
		 */
		try {
			/*
			 * dao에 create table SQL 을 작성해 두고
