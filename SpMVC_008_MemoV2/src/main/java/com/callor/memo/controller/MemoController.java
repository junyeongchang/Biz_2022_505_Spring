...
public class MemoController {
	...
	/*
	 * 첨부 파일이 있는 프로젝트에서 form 의 file input box type 에 이름은 절대 VO(DTO) 클래스에 선언된 이름을
	 * 사용하면 안된다.
	 */
	...
	public String update(@PathVariable String seq,
			@ModelAttribute("memo") MemoDTO memoDTO, MultipartFile file, HttpSession httpSession) {
		
		String username = (String)httpSession.getAttribute("USERNAME");
		if(username== null) {
			return " redirect:/user/login";
		}
		// 로그인된 사용자의 이름을 memoDTO에 settion 하기
	...
	private MemoDTO memoDTO() {
		...
		MemoDTO memo = MemoDTO.builder().m_date(toDate.format(date)).m_time(toTime.format(date)).build();
		return memo;
	}
}
